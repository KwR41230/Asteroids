...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    # Scale background to fit window (640 / 640 = 1.0, 480 / 441 = ~1.08)
  ...
  def save_game
    data = {
      ...
      player: { x: @player.x, y: @player.y, shield_until: @player.shield_until, weapon_type: @player.weapon_type, weapon_until: @player.weapon_until },
  ...
  def draw
    ...
    Gosu.translate(off_x, off_y) do
      # Draw background scaled to fit window
      @background_image.draw(0, 0, 0, 1.0, @bg_scale_y)
      @starfield.draw
  ...
  def draw_menu
    @font_large.draw_text("ASTEROIDS", 160, 100, 2)
    @menu_options.each_with_index do |opt, i|
      ...
      @font_small.draw_text(opt, 320 - (@font_small.text_width(opt)*scale/2), 200 + (i * 40), 2, scale, scale, color)
    end
  end

  def draw_pause_overlay
    Gosu.draw_rect(0, 0, WIDTH, HEIGHT, Gosu::Color.rgba(0, 0, 0, 150), 10)
    @font_large.draw_text("PAUSED", 210, 100, 11, 1, 1, Gosu::Color::YELLOW)
    @pause_options.each_with_index do |opt, i|
      color = (i == @pause_index) ? Gosu::Color::YELLOW : Gosu::Color::WHITE
      scale = (i == @pause_index) ? 1.2 : 1.0
      @font_small.draw_text(opt, 320 - (@font_small.text_width(opt)*scale/2), 220 + (i * 40), 11, scale, scale, color)
    end
  end

  def draw_high_scores
    @font_large.draw_text("HIGH SCORES", 140, 50, 2, 1, 1, Gosu::Color::YELLOW)
    HighScores.load.each_with_index do |s, i|
      y = 130 + (i * 30)
      @font_small.draw_text("#{i+1}. #{s['name']}", 200, y, 2); @font_small.draw_text(s['score'].to_s.rjust(5), 400, y, 2, 1, 1, Gosu::Color::CYAN)
    end
    @font.draw_text("Press ESC for Menu", 210, 440, 2, 1, 1, Gosu::Color::GRAY)
  end

  def draw_name_entry
    @font_large.draw_text("NEW HIGH SCORE!", 80, 100, 2, 0.8, 0.8, Gosu::Color::YELLOW)
    3.times do |i|
      ...
      @font_large.draw_text(char, 260+(i*50), 260, 2, scale, scale, color)
    end
  end
  ...
  def draw_gameplay
    ...
    if @player.shielded?
      ...
      bar_w = ([0, @player.shield_until - Gosu.milliseconds].max / 10000.0) * 100
      Gosu.draw_rect(WIDTH-120, 20, 104, 14, Gosu::Color::WHITE, 2); Gosu.draw_rect(WIDTH-118, 22, bar_w, 10, Gosu::Color::CYAN, 2)
    elsif Gosu.milliseconds < @shield_cooldown_until
      bar_w = ((@shield_cooldown_until - Gosu.milliseconds) / 30000.0) * 100
      Gosu.draw_rect(WIDTH-120, 20, 104, 14, Gosu::Color.rgba(100,100,100,255), 2); Gosu.draw_rect(WIDTH-118, 22, bar_w, 10, Gosu::Color::RED, 2)
    elsif @combo_count > 0
      bar_w = (@combo_count / 5.0) * 100
      Gosu.draw_rect(WIDTH-120, 20, 104, 14, Gosu::Color::GRAY, 2); Gosu.draw_rect(WIDTH-118, 22, bar_w, 10, Gosu::Color::YELLOW, 2)
    end
    if @player.weapon_type != :normal
      bar_w = ([0, @player.weapon_until - Gosu.milliseconds].max / 10000.0) * 100
      color = @player.weapon_type == :spread ? Gosu::Color::RED : Gosu::Color::GREEN
      Gosu.draw_rect(WIDTH-120, 45, 104, 14, Gosu::Color::WHITE, 2); Gosu.draw_rect(WIDTH-118, 47, bar_w, 10, color, 2)
    end
    @font.draw_text("Score: #{@score}", 10, 10, 2, 1, 1, Gosu::Color::YELLOW); @font.draw_text("Level: #{@level}", 10, 30, 2, 1, 1, Gosu::Color::YELLOW)
    @lives.times { |i| @ship_image.draw_rot(30 + (i * 35), HEIGHT - 30, 2, 0, 0.5, 0.5, 0.5, 0.5) }
    if @state == :level_up
      @font_large.draw_text("LEVEL COMPLETE!", 100, 150, 2, 1, 1, Gosu::Color::YELLOW) if (Gosu.milliseconds/500)%2 == 0
      @font_small.draw_text("Get ready for Level #{@level+1}...", 190, 220, 2)
    end
  end

  def draw_game_over
    @font_large.draw_text("GAME OVER", 150, 150, 2, 1, 1, Gosu::Color::RED)
    @font_small.draw_text("Final Score: #{@score}", 230, 220, 2); @font_small.draw_text("Press SPACE to continue", 190, 280, 2, 1, 1, Gosu::Color::YELLOW)
  end
  ...
  def handle_asteroid_destruction(asteroid)
    ...
    (@state = :level_up; @level_up_timer = Gosu.milliseconds + 2000; @level_up_sound.play(0.5)) if @asteroids_cleared >= 15
  end
  ...
  def update
    ...
    if @state == :level_up
      (@level += 1; @asteroids_cleared = 0; @asteroids, @bullets, @ufos, @alien_bullets = [], [], [], []; @state = :playing) if Gosu.milliseconds > @level_up_timer
      return
    end
    ...
    @bullets.reject! do |bullet|
      ...
      if hit_ast
        ...
        create_explosion(hit_ufo.x, hit_ufo.y, 15); @ufos.delete(hit_ufo); @score += 50; @explosion_sound.play; true
      else; false; end
    end
    ...
    unless @player.recovering?
      ...
      if hit_ast || hit_ufo || hit_bul
        if @player.shielded?
          ...
          @shake_amount = 15
          @state = :game_over if @lives <= 0
        end
      end
    end
    
    if rand(100) < (1 + (@level * 1)) then img = rand < 0.5 ? @asteroid_small_image : @asteroid_medium_image; @asteroids << Asteroid.new(img, 2 + (@level * 0.5), (img == @asteroid_small_image ? 12 : 28)) end
    if rand(1000) < (2 + @level) && @ufos.empty? then @ufos << UFO.new(@ufo_image, @alien_bullet_image, rand(0..1)) end
  end
end
